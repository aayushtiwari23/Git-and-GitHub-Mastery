...
A matrix build allows one GitHub Actions job to run multiple times with different configurations.

For example, you may want to test your application with:
...
Instead of creating three separate jobs, you can use a matrix.
...
Matrix builds are useful when you need to test the same code in different environments.
...
## 3. Basic Matrix Syntax

A matrix is defined under:
...
## 4. Matrix With Operating Systems

You can use a matrix to test different operating systems.
...
and `runs-on` uses that value.

---

## 5. Matrix With Programming Versions
...
The job runs once for each Python version.

---

## 6. Matrix Values

A matrix can contain multiple values.
...
GitHub creates combinations of these values.
