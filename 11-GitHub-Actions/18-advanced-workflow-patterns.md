...
Advanced workflow patterns are techniques used to make GitHub Actions workflows:
...
At this stage, the goal is to combine the concepts learned so far.

---

## 2. Basic Workflow Pattern

A simple CI workflow might look like:
...
Instead of putting everything into one job, separate responsibilities.
...
Jobs don't always need to run sequentially.
...
Parallel jobs can reduce total workflow time.

---

## 5. Combining Parallel and Sequential Jobs
...
```text
       ┌── Lint ──┐
       │          │
Start ─┼── Test ──┼──→ Build ──→ Deploy
       │          │
       └── Scan ──┘
```

The later job can depend on all required jobs.
