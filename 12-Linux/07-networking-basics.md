...
A Linux system can communicate through:
...
A network interface is the connection through which a device communicates with a network.
...
Modern Linux systems often use predictable names such as:
...
The `ip` command is one of the main tools for network configuration and inspection.
...
The loopback interface allows the computer to communicate with itself.
...
usually refers to the local computer.
